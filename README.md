...
This starter template offers two ways to build your components: use the included GitHub Actions for a no-setup approach, or set up a local development environment for faster iteration. With the GitHub Actions approach, simply commit to the main branch and your components are automatically built and hosted. All you need is the base URL to start using your library on any website, with new versions fetched automatically. For rapid development, the local setup creates a secure tunnel to your localhost and provides hot reload functionality, letting you see changes in real-time.
